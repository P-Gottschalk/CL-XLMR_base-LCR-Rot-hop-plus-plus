...
def main():
    ...
    if not generate_all:
        ...
        embeddings_layer = EmbeddingsLayer(hops=ont_hops, ontology=ontology, use_vm=use_vm, use_soft_pos=use_soft_pos,
                                           device=device)
        embeddings_dir = EmbeddingsDataset(model="mBERT", year=year, device=device, phase=phase, language=language, ont_hops=ont_hops, empty_ok=True,
    ...
    for language in ['English', 'Dutch', 'French', 'Spanish']:
        for phase in ['Train', 'Test']:
            ...
            embeddings_dir = EmbeddingsDataset(model="mBERT", year=year, device=device, phase=phase, language=language, ont_hops=None, empty_ok=True).dir
            generate_embeddings(embeddings_layer,data,embeddings_dir)
            ...
            for ont_hops in range(3):
                for use_vm in [True, False]:
                    for use_soft_pos in [True, False]:
                        ...
                        embeddings_dir = EmbeddingsDataset(model="mBERT", year=year, device=device, phase=phase, ont_hops=ont_hops,
